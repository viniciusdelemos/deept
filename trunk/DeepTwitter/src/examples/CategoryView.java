...
public class CategoryView extends Display{
    ...
    public CategoryView() {
        ...
        ForceDirectedLayout fdl = new ForceDirectedLayout(graphNodesAndEdges, false);
        final GraphDistanceFilter filter = new GraphDistanceFilter(graphNodesAndEdges, hops);
...
//      makes us able to stop TextEditor by special KeyEvents (e.g. Enter)
    ...
    private void initDataGroups() {
        ...
        {
	        Node n10 = g.addNode();
	        n10.setString(VisualItem.LABEL, "Node "+i);
    ...
    */
    ...
    class MoveObjectsGood extends Thread{
    	...
    	private double fy(double a, double b, double fa, double fb, double x){
    		...
    		double ret = (((b - x)/(b-a)) * fa) + (((x-a)/(b-a))/fb);
    	...
    	private void primeiraPosicao(){
			...
			while(nodes.hasNext()){
				...
				double xx = (50 + Math.random()*(display.getBounds().width - 100));
				double yy = (50 + Math.random()* (display.getBounds().height - 100));
				
				PosicaoDesejada posicaoDesejada =
					new PosicaoDesejada(t, dX, dY);
    	...
    	private void novaPosicao(PosicaoDesejada p){
    		...
    		VisualItem visualItem = getVisualization().getVisualItem(NODES, p.node);
    		...
    		if(visualItem.getX() < (display.getWidth() / 2)
    			...
    			p.dY = (int) ((display.getHeight() / 2) + (Math.random() * (display.getHeight() / 2))); //>
    			
    			System.out.println("-> Primeiro quadrante: "+ p.dX+" "+p.dY);
    		}
    		...
    		//segundo quadrante
    			...
    			p.dX = (int)((display.getWidth() / 2) + (Math.random() * (display.getWidth() / 2))); //>
    			p.dY = (int) ((display.getHeight() / 2) + (Math.random() * (display.getHeight() / 2))); //>
    			
    			System.out.println("-> Segundo quadrante: "+ p.dX+" "+p.dY);
    		}
    		...
    		//terceiro quadrante
    			...
    			System.out.println("-> Terceiro quadrante: "+ p.dX+" "+p.dY);
    		}
    	...
    	public void run(){
    		...
    		while(true){
    			...
    			for(PosicaoDesejada p : posicoesObj){
    				...
    				System.out.println("Display x:"+display.getWidth() + " y:"+display.getHeight() 
    						+" Objeto x:"+visualItem.getX() +" y:"+visualItem.getY());
    		...
    		while(condicao){
    			...
    			for(PosicaoDesejada p : posicoes){
    				...
    				if(visualItem.getBounds().getX() < contorno || 
    						visualItem.getBounds().getX() > display.getWidth() - contorno ||
    						visualItem.getBounds().getY() < contorno || 
    						visualItem.getBounds().getY() > display.getHeight() - contorno){
    				...
    				if(verticalhorizontal == 0 
    						...
    						//visualItem.getBounds().getX() <= display.getBounds().getWidth()
    						){
    					...
    					//direira
    						...
    						//visualItem.getBounds().getY() <= display.getBounds().getHeight()
    						){
