 ...
 * which drags all focused items around.
 * 
 * First version - it would by more handy to limit this multiple dragging to the cases when the
 * control key is pressed while dragging ....
 * 
 * @author goose
 *
 */

public class GraphViewMultipleDragV2 extends JPanel {
    ...
    public GraphViewMultipleDragV2(Graph g, String label) {
...
//	opanel.add(overview);
	...
	final GraphViewMultipleDragV2 view = new GraphViewMultipleDragV2(g, label);
	...
	private GraphViewMultipleDragV2 m_view;
	public GraphMenuAction(String name, String accel, GraphViewMultipleDragV2 view) {
	    m_view = view;
	...
	public OpenGraphAction(GraphViewMultipleDragV2 view) {
	    m_view = view;
	...
	private void moveIt(VisualItem vi,double dx, double dy) {
	    ...
	    vi.setStartX(x);  vi.setStartY(y);
	    vi.setX(x+dx);    vi.setY(y+dy);
	    vi.setEndX(x+dx); vi.setEndY(y+dy);
	}
